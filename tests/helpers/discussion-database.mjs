...
export async function discussionDatabase(overrides = {}) {
  /** @type {Record<string,string>} */
  ...
  await db.exec(`
    ...
    create table unit_sale_document_versions(id uuid primary key default gen_random_uuid(),document_id uuid references unit_sale_documents,version_number integer,is_current boolean default true,
      file_name text,redacted_at timestamptz,uploaded_by_user_id uuid);
    ...
    create function can_access_sales_building(b uuid) returns boolean language sql stable security definer as $$select is_sales_internal_user() or exists(select 1 from user_building_access where user_id=auth.uid() and building_id=b)$$;
    create function can_access_sale_attempt(a uuid) returns boolean language sql stable security definer as $$select exists(select 1 from unit_sale_attempts where id=a and can_access_sales_building(building_id))$$;
