...
async function legalFixture(page: Page) {
  ...
  await page.route("**/api/sales/legal**", async (route) => {
    ...
    if (route.request().headers()["content-type"]?.includes("multipart/form-data")) {
      ...
      if (form.get("action") === "upload_completion_documents") {
        const files=form.getAll("files") as File[], assignments=JSON.parse(String(form.get("assignments"))) as {type:string;expectedVersionId:string|null}[];
        actions.push({action:"upload_completion_documents",names:files.map(file=>file.name),assignments});
        if(failNotice){await route.fulfill({status:400,json:{error:"Upload failed; please retry."}});return;}
        files.forEach((file,index)=>{
          const type=assignments[index].type;let document=f.rows.unit_sale_documents.find(item=>item.document_type===type);
          ...
          const previous=f.rows.unit_sale_document_versions.filter(version=>version.document_id===document.id);previous.forEach(version=>{version.is_current=false;});
          f.rows.unit_sale_document_versions.push({id:type+"-"+(previous.length+1),document_id:document.id,version_number:previous.length+1,is_current:true,file_name:file.name,file_size_bytes:file.size,uploaded_at:new Date().toISOString(),uploaded_by_user_id:userId});
        });
        f.completionPackage.approved=false;await route.fulfill({json:{}});return;
      }
...
test("completion package selects both PDFs, queries one file, preserves replacement history and unlocks legal completion only after approval",async({page})=>{
  ...
  expect(f.actions.filter(action=>action.action==="upload_completion_documents")).toHaveLength(2);expect(f.rows.unit_sale_document_versions).toHaveLength(2);await documents.screenshot({style:"nextjs-portal { display: none !important; }",path:"artifacts/completion-package/03-uploaded-awaiting.png"});
...
test("completion combined drop zone keeps both files and independent upload leaves developer approval locked",async({page})=>{
  ...
  expect(f.rows.unit_sale_document_versions).toHaveLength(1);await documents.getByLabel("Choose completion documents",{exact:true}).setInputFiles({name:"invalid.txt",mimeType:"text/plain",buffer:Buffer.from("text")});await expect(documents.getByRole("alert")).toContainText("PDF");
