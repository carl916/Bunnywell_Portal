alter table public.unit_sale_attempts
  add column if not exists buyer_person_name text,
  add column if not exists buyer_company_name text,
  add column if not exists reservation_terms_checked boolean not null default false,
  add column if not exists reservation_submitted_by_user_id uuid references public.profiles(id),
  add column if not exists reservation_submitted_by_name text,
  add column if not exists reservation_submitted_by_email text;

update public.unit_sale_attempts
set buyer_person_name = buyer_name
where buyer_person_name is null
  and buyer_name is not null;

alter table public.unit_sale_terms
  add column if not exists developer_contribution_value numeric(14, 4),
  add column if not exists developer_contribution_value_type text not null default 'amount',
  add column if not exists agent_contribution_value numeric(14, 4),
  add column if not exists agent_contribution_value_type text not null default 'amount',
  add column if not exists parking_contribution_value numeric(14, 2) not null default 0,
  add column if not exists parking_location_details text,
  add column if not exists additional_special_conditions text[] not null default '{}'::text[];

update public.unit_sale_terms
set
  developer_contribution_value = coalesce(developer_contribution_value, developer_contribution),
  agent_contribution_value = coalesce(agent_contribution_value, agent_contribution)
where developer_contribution_value is null
   or agent_contribution_value is null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'unit_sale_attempts_buyer_identity_check'
      and conrelid = 'public.unit_sale_attempts'::regclass
  ) then
    alter table public.unit_sale_attempts
      add constraint unit_sale_attempts_buyer_identity_check check (
        workflow_status not in ('reservation_submitted', 'reservation_approved', 'awaiting_commercial_approval', 'ready_for_exchange', 'exchanged', 'completion_pending', 'completed')
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'unit_sale_terms_contribution_type_check'
      and conrelid = 'public.unit_sale_terms'::regclass
  ) then
    alter table public.unit_sale_terms
      add constraint unit_sale_terms_contribution_type_check check (
        developer_contribution_value_type in ('amount', 'percent')
        and agent_contribution_value_type in ('amount', 'percent')
      );
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'unit_sale_terms_buyer_incentives_check'
      and conrelid = 'public.unit_sale_terms'::regclass
  ) then
    alter table public.unit_sale_terms
      add constraint unit_sale_terms_buyer_incentives_check check (
        (developer_contribution_value is null or developer_contribution_value >= 0)
        and (agent_contribution_value is null or agent_contribution_value >= 0)
        and parking_contribution_value >= 0
        and (developer_contribution_value_type <> 'percent' or developer_contribution_value <= 100)
        and (agent_contribution_value_type <> 'percent' or agent_contribution_value <= 100)
      );
  end if;
end $$;
...
begin
  ...
  if found then
    ...
    set
      ...
      developer_contribution_value = coalesce(p_developer_contribution_value, p_developer_contribution, 0),
      developer_contribution_value_type = coalesce(p_developer_contribution_value_type, 'amount'),
      agent_contribution_value = coalesce(p_agent_contribution_value, p_agent_contribution, 0),
      agent_contribution_value_type = coalesce(p_agent_contribution_value_type, 'amount'),
      parking_contribution_value = coalesce(p_parking_contribution_value, 0),
      parking_location_details = p_parking_location_details,
      additional_special_conditions = coalesce(p_additional_special_conditions, '{}'::text[]),
    ...
    insert into public.unit_sale_terms (
      ...
      developer_contribution_value_type,
      agent_contribution_value,
      agent_contribution_value_type,
      parking_contribution_value,
      parking_location_details,
      additional_special_conditions,
      reservation_fee,
    ...
    values (
      ...
      coalesce(p_developer_contribution_value, p_developer_contribution, 0),
      coalesce(p_developer_contribution_value_type, 'amount'),
      coalesce(p_agent_contribution_value, p_agent_contribution, 0),
      coalesce(p_agent_contribution_value_type, 'amount'),
      coalesce(p_parking_contribution_value, 0),
      p_parking_location_details,
      coalesce(p_additional_special_conditions, '{}'::text[]),
      p_reservation_fee,
