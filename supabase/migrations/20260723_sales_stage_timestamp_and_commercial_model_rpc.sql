...
set stage_entered_at = coalesce(
  case
    when workflow_status = 'completed' then completed_at::timestamptz
    when workflow_status in ('exchanged', 'completion_pending') then exchanged_at::timestamptz
    when workflow_status in ('reservation_approved', 'awaiting_commercial_approval', 'ready_for_exchange') then reservation_approved_at
    when workflow_status in ('reservation_submitted', 'reservation_query_raised') then reservation_submitted_at
    when workflow_status = 'fallen_through' then fallen_through_at
    else null
  end,
  created_at
)
...
create or replace function public.set_sale_attempt_stage_entered_at()
returns trigger
language plpgsql
as $$
begin
  if tg_op = 'INSERT' then
    ...
    new.stage_entered_at = coalesce(new.stage_entered_at, old.stage_entered_at);
  end if;
...
drop trigger if exists set_unit_sale_attempts_stage_entered_at on public.unit_sale_attempts;
create trigger set_unit_sale_attempts_stage_entered_at
before insert or update on public.unit_sale_attempts
for each row execute function public.set_sale_attempt_stage_entered_at();
...
begin
  ...
  if not found then
    ...
    values (
      v_unit.building_id,
      v_unit.id,
      coalesce((select max(attempt_number) + 1 from public.unit_sale_attempts where unit_id = v_unit.id), 1),
      'draft',
      true,
      v_now,
      p_requester_id,
