...
Comments support plain text, safe HTTP(S) links, stage context, flat replies, eligible user-ID mentions, author edits, revision inspection, date separators and independently scrolling history. The composer keeps its draft until a confirmed send. A client operation UUID and database uniqueness constraint protect retries; edited retries cannot silently discard revised text. The client and database enforce a 5,000-character limit and reject whitespace-only messages. Enter inserts a newline; Ctrl/Cmd+Enter sends. Drafts are stored in session storage under the authenticated user and transaction IDs. Closing, switching workspace/stage or changing presentation preserves them.
...
New `sale_participants` assignments are individual and transaction-specific. Current building eligibility plus that assignment is required for agents and solicitors. Existing active developer/admin access is preserved. Free-text buyer/solicitor fields and organisation membership alone never confer discussion access. Recorded creators and reservation submitters are enrolled individually; further participants are assigned or revoked through **Sale participants** by developers/admins. Existing records only bootstrap genuinely recorded creator/submitter IDs, not everyone in their organisations.
...
The database tests execute both new migrations unchanged in PGlite with an isolated adapter for the existing Supabase schema/auth functions, including the repository's enum role type. They exercise RLS, individual assignments, wrong-sale denial, access revocation, draft/replacement isolation, edits/history, reply validation, retry protection, mention deduplication, exact read receipts and monotonic state, document triggers, safe projections and signed-document access checks. Playwright uses synthetic auth/data and routes discussion RPC calls into that local PostgreSQL instance. Existing workflow browser fixtures also remain synthetic. No test sends real email or queries a live conversation.
